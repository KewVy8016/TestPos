<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if($row['member_username'] > 0){
           ...
           echo "<script>window.location.href='add-usr.php' </script>";
        }elseif(!$check_pass){
            echo "<script>alert('ยืนยันรหัสไม่ตรงกัน') </script>";
           echo "<script>window.location.href='add-usr.php' </script>";
        }else{
            $pass_md = md5($pass);
            $s_in ="INSERT INTO member(member_id, member_img, member_username, member_name, member_lastname, member_password, member_level, member_status) VALUES (Null,Null,'$username','$name','$lastname','$pass_md','member','access')";
